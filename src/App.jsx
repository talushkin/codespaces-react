...
function App() {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      if (!res.ok) {
        const msg = body?.messages?.join(', ') || text || `Login failed: ${res.status}`;
        throw new Error(msg);
      }
  ...
  const refreshAccessToken = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        const message = parsed?.messages?.join(', ') || `Refresh failed: ${res.status}`;
    ...
    try {
      ...
      if (res.status === 500 && !attemptedRefresh) {
        ...
        if (!retryRes.ok) {
          ...
          throw new Error(retryBody?.messages?.join(', ') || `Projects failed: ${retryRes.status}`);
        }
      ...
      throw new Error(body?.messages?.join(', ') || `Projects failed: ${res.status}`);
    } catch (err) {
      setError(err.message);
    } finally {
      setLoading(false);
    ...
    <div className="page">
      <div className="card">
        ...
        <div className="actions">
          ...
          <button onClick={handleGetProjects} disabled={loading || !accessToken}>
